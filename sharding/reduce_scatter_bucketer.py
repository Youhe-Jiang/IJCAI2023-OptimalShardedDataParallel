...
from sharding.comm_ops.reduce_scatter_with_groups import dist_group_reduce_scatter
...
class Bucket:
    ...
    def flush(
            self, 
            choice: bool, 
            choice_reduce_scatter: bool, 
            group_all_reduce: ProcessGroup, 
            group_intra_node: ProcessGroup, 
            group_inter_node: ProcessGroup, 
            inter_cache: List, 
            inter_cache_type: List
        ) -> None:
        ...
        if hasattr(dist, "_reduce_scatter_base") and enable_nccl_base_collectives:
            if choice_reduce_scatter:
                dist_group_reduce_scatter(
                        output=self.output_shard[: self.offset], 
                        inter_cache=inter_cache, 
                        inter_cache_type=inter_cache_type,
                        input=self.data[:, : self.offset].contiguous(), 
                        intra_group=group_intra_node, 
                        inter_group=group_inter_node,
                )
            else:
            ...
            if choice:
                dist.all_reduce(self.output_shard[: self.offset], group=group_all_reduce)
        else:
            dist.reduce_scatter(
...
class ReduceScatterBucketer:
    ...
    def reduce_scatter_async(
        ...
        choice: bool = False,
        choice_reduce_scatter: bool = False,
        group_all_reduce: Optional[ProcessGroup] = None,
        group_intra_node: Optional[ProcessGroup] = None,
        group_inter_node: Optional[ProcessGroup] = None,
        ...
        if first_input_size > bucket_shard_size:
            ...
            if hasattr(dist, "_reduce_scatter_base") and enable_nccl_base_collectives:
                ...
                if choice:
                    dist.all_reduce(output, group=group_all_reduce)
            else:
                # fallback
        ...
        if first_input_size > bucket.data.size(1) - bucket.offset:
            ...
            bucket.flush(choice, choice_reduce_scatter, group_all_reduce, group_intra_node, group_inter_node, inter_cache, inter_cache_type)
