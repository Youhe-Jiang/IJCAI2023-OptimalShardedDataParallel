...
from sharding.comm_ops.all_gather_with_groups import dist_group_all_gather
from sharding.reduce_scatter_bucketer import ReduceScatterBucketer
...
class FullyShardedDataParallel(nn.Module):
    ...
    def __init__(
        ...
        process_group_all_reduce: Optional[ProcessGroup] = None,
        all_reduce_before_backward: bool = False,
        process_group_intra_node: Optional[ProcessGroup] = None,
        process_group_inter_node: Optional[ProcessGroup] = None,
        inter_cache: Optional[List] = None,
        inter_cache_type: Optional[List] = None,
        inter_cache_reduce_scatter: Optional[List] = None,
        inter_cache_reduce_scatter_type: Optional[List] = None,
        communication_with_groups: bool = False,
        ...
        # All_reduce before backward
        self.process_group_all_reduce = process_group_all_reduce
        self.all_reduce_before_backward = all_reduce_before_backward
        # Communication with groups
        self.process_group_intra_node = process_group_intra_node
        self.process_group_inter_node = process_group_inter_node
        self.inter_cache = inter_cache
        self.inter_cache_type = inter_cache_type
        self.inter_cache_reduce_scatter = inter_cache_reduce_scatter
        self.inter_cache_reduce_scatter_type = inter_cache_reduce_scatter_type
        self.communication_with_groups = communication_with_groups
        ...
        with torch.cuda.stream(self._streams["post_backward"]):
            ...
            if param._is_sharded:
                ...
                self._reducer.reduce_scatter_async(
                        ...
                        choice=self.all_reduce_before_backward, 
                        choice_reduce_scatter=self.communication_with_groups,
                        group_all_reduce=self.process_group_all_reduce,
                        group_intra_node=self.process_group_intra_node,
                        group_inter_node=self.process_group_inter_node,
                        inter_cache=self.inter_cache_reduce_scatter,
                        inter_cache_type=self.inter_cache_reduce_scatter_type,
                )
            else:
            ...
            for p in self.params:
                if not p._is_sharded:  # e.g., when world_size == 1
                    ...
                    if hasattr(dist, "_all_gather_base") and enable_nccl_base_collectives:
                        ...
                        if self.communication_with_groups:
                            dist_group_all_gather(
                                    output=output_tensor, 
                                    inter_cache=self.inter_cache,
                                    inter_cache_type=self.inter_cache_type,
                                    input=p_data, 
                                    intra_group=self.process_group_intra_node, 
                                    inter_group=self.process_group_inter_node,
                            )
                        else:
