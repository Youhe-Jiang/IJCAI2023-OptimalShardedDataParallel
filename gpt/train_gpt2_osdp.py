...
from dataset.AdditionDataset import AdditionDataset
...
def train(args):
    if args.fsdp_type == 'OSDP':
        from sharded_data_parallel.optimal_sharded_data_parallel import OptimalShardedDataParallel as OSDP
        ...
        if it >= 7:
            if rank == 0 and args.profile == 1:
                print('System throughput: ', 
                        args.train_batch_size * args.seq_length * 6 / ((end_time-start_time) / 3))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--fsdp_type", type=str, default='OSDP', help="Type of FSDP"
    )
    parser.add_argument(
